...
func CheckBestBuy(client *http.Client, findSkusFromWeb bool){
  ...
  for {
    ...
    foundMatch := false
    for k, v := range bestBuyStatuses {
      if v {
        url := fmt.Sprintf("https://www.bestbuy.com/site/searchpage.jsp?st=%v&_dyncharset=UTF-8&_dynSessConf=&id=pcat17071&type=page&sc=Global&cp=1&nrp=&sp=&qp=&list=n&af=true&iht=y&usc=All+Categories&ks=960&keys=keys", k)
...
func parseHtmlForStatus(sku string, resp *http.Response) bool{
  ...
  if err != nil || resp.StatusCode != 200 {
    log.Println("unable to parse HTML for status code, assuming  %v out of stock", sku)
    return false
  }
